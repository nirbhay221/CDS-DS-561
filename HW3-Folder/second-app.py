...
def callback(message : pubsub_v1.subscriber.message.Message)-> None:
    try:
        ...
        headers = pubsub_message.get("headers", {}) # Extract headers from the request received
        country = headers.get("X-Country", "Unknown") # Country is extracted from the header
 
        # Process the message 
        if country != 'Unknown': # check if the country exist or not
            print(f"Received forbidden request from country: {country}")
            print("Error: Access is prohibited. Error 403") # Error Message
        else:
