    ...
    if method != "GET":
        ...
        if country in banned_countries:
            ...
            if contents is not None:
                ...
                # Get the GCP zone and set it as a response header
                zone = get_gcp_zone()
                if zone:
                    print(f"GCP Zone: {zone}")
                    return contents, 200, {'GCP-Zone': zone}
                else:
                    print("Unable to determine GCP Zone")
...
def get_gcp_zone():
    try:
        credentials, project = compute_engine.Credentials(), 'cdsds561-project-1'
        zone_url = 'http://metadata.google.internal/computeMetadata/v1/instance/zone'
        
        # Create a dictionary of headers
        headers = {'Metadata-Flavor': 'Google'}
        
        # Use the `headers` parameter to pass the headers
        response = requests.get(zone_url, headers=headers)
        
        zone_path = response.text.split('/')[-1]
        return zone_path.split('/')[-1]
    except Exception as e:
        print(f"Error fetching GCP Zone: {str(e)}")
        return None
